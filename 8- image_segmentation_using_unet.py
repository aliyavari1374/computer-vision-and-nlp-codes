...
TRAIN_IMAGE_PATH = '/content//drive//My Drive//Colab Notebooks//Breast//Dataset//Inputs_Train/'
TRAIN_MASK_PATH = '/content//drive//My Drive//Colab Notebooks//Breast//Dataset//Masks_Train'
TEST_IMAGE_PATH = '/content//drive//My Drive//Colab Notebooks//Breast//Dataset//Inputs_Test/'
TEST_MASK_PATH = '/content//drive//My Drive//Colab Notebooks//Breast//Dataset//Masks_Test'

# 3. Initialize Images and Mask Size
IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS = 768, 896, 3
...
def Data_Proprocessing_Train():
    ...
    Init_Image = np.zeros((len(Train_Mask_List), 768, 896, 3), dtype = np.uint8) # original image
    Init_Mask = np.zeros((len(Train_Mask_List), 768, 896), dtype = np.bool) # original mask
    Train_X = np.zeros((len(Train_Mask_List), IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS), dtype = np.uint8) # patch
    Train_Y = np.zeros((len(Train_Mask_List), IMG_HEIGHT, IMG_WIDTH, 1), dtype = np.bool) # patch mask
    ...
    for mask_path in glob.glob('{}/*.TIF'.format(TRAIN_MASK_PATH)):
        ...
        y_coord, x_coord = np.where(mask == 255) # white pixels
        ...
        Train_X[n] = resize(cropped_image[:,:,:IMG_CHANNELS],(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS), mode = 'constant', anti_aliasing=True, preserve_range=True)
        
        # one-hot encoding
        Train_Y[n] = np.expand_dims(resize(cropped_mask,(IMG_HEIGHT, IMG_WIDTH), mode = 'constant',anti_aliasing=True, preserve_range=True), axis = -1)
...
imshow(np.squeeze(Train_Masks[0])) # one-hot encoded image => np.squeeze
plt.show()
...
def U_Net_Segmentation(input_size=(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS)):
    ...
    # b6 => Decoding 
    u6 = Conv2DTranspose(128, (2,2), strides=(2,2), padding='same')(c5) # up convolution
    u6 = concatenate([u6, c4]) # a concatenate in u-net figure
...
imageset = 'BCC' #Breast Cancer Cell
backbone = 'UNET'
version = 'v1.0'
